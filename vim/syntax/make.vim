...
syn region makeDefine start="^\s*define\s" end="^\s*endef\s*\(#.*\)\?$" contains=makeStatement,makeIdent,makePreCondit,makeDefine,makeComment
...
syn match makeIdent	"\$[^({]"
syn match makeIdent	"^\s*\w[-.A-Za-z0-9_/]*\s*[:+?!*]="me=e-2
syn match makeIdent	"^\s*\w[-.A-Za-z0-9_/]*\s*="me=e-1
syn match makeIdent	"%"
